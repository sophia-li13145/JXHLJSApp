...
public partial class ProductionQualityOrderListPage : ContentPage
{
    ...
    private async void OnScanManualMaterialClicked(object sender, TappedEventArgs e)
    {
        ...
        try
        {
            ...
            await Shell.Current.GoToAsync($"{AppShell.RouteMachineQualityDetail}?qualityNo={Uri.EscapeDataString(qualityNo)}&workOrderNo={Uri.EscapeDataString(detail.workOrderNo ?? string.Empty)}&inspectStatus={Uri.EscapeDataString(detail.inspectStatus ?? string.Empty)}");
        }
        catch (Exception ex)
        {
